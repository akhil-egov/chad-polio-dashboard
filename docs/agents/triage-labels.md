# Triage Labels — chad-polio-dashboard

These five labels encode workflow state. They are separate from category labels (`bug`, `map`, etc.).
...
- `ready-for-agent` requires: clear acceptance criteria, no open questions, all referenced files/fields exist.
