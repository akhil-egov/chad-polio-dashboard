  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total Issued', value: totalIssued, color: '#009FDB' },
          { label: 'Total Returned', value: totalReturned, color: '#ca8a04' },
          { label: 'Total Used', value: totalUsed, color: '#16a34a' },
        ].map(({ label, value, color }) => (
          <div key={label} className="bg-white border border-slate-200 rounded-md shadow-sm overflow-hidden">
            <div className="h-[3px]" style={{ background: color }} />
            <div className="px-5 py-4">
              <p className="font-condensed text-[9px] font-bold tracking-[0.2em] uppercase text-slate-400">{label}</p>
              <p className="font-data text-[1.5rem] font-bold text-slate-800 leading-none mt-1">{value.toLocaleString()}</p>
              <p className="font-condensed text-[9px] text-slate-400 mt-1 uppercase tracking-wide">vials</p>
            </div>
          </div>
        ))}
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50">
              ...
              <th className={th} onClick={() => handleSort('vials_issued')}>Issued <SortIcon k="vials_issued" /></th>
              <th className={th} onClick={() => handleSort('vials_returned')}>Returned <SortIcon k="vials_returned" /></th>
              <th className={th} onClick={() => handleSort('vials_used')}>Used <SortIcon k="vials_used" /></th>
              <th className={th} onClick={() => handleSort('utilization')}>Utilization <SortIcon k="utilization" /></th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => {
              const utilColor = r.utilization >= 80 ? 'text-green-700' : r.utilization >= 50 ? 'text-amber-600' : 'text-red-600'
              return (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="w-16 bg-slate-200 rounded-full h-[4px]">
                        <div className="h-[4px] rounded-full" style={{ width: `${Math.min(r.utilization, 100)}%`, background: r.utilization >= 80 ? '#16a34a' : r.utilization >= 50 ? '#ca8a04' : '#dc2626' }} />
                      </div>
