...
export function MicroplanTable() {
  ...
  const th = 'px-4 py-3 text-left font-condensed text-[9px] font-bold tracking-[0.18em] uppercase text-slate-500 cursor-pointer select-none hover:text-[#009FDB] transition-colors whitespace-nowrap'
  const thStatic = 'px-4 py-3 text-left font-condensed text-[9px] font-bold tracking-[0.18em] uppercase text-slate-500 whitespace-nowrap'
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded-md shadow-sm px-5 py-4 flex flex-wrap gap-6 items-center">
        <div>
          <p className="font-condensed text-[9px] font-bold tracking-[0.2em] uppercase text-slate-400">Campaign Target</p>
          <p className="font-data text-[1.5rem] font-bold text-slate-800 leading-none mt-1">{totalTarget.toLocaleString()}</p>
        </div>
        <div>
          <p className="font-condensed text-[9px] font-bold tracking-[0.2em] uppercase text-slate-400">Achieved</p>
          <p className="font-data text-[1.5rem] font-bold text-slate-800 leading-none mt-1">{totalAchieved.toLocaleString()}</p>
        </div>
        <div>
          <p className="font-condensed text-[9px] font-bold tracking-[0.2em] uppercase text-slate-400">Gap</p>
          <p className="font-data text-[1.5rem] font-bold text-red-600 leading-none mt-1">{(totalTarget - totalAchieved).toLocaleString()}</p>
        </div>
        <div className="flex-1 min-w-[180px]">
          <p className="font-condensed text-[9px] font-bold tracking-[0.2em] uppercase text-slate-400 mb-2">Overall Progress</p>
          ...
          <p className="font-data text-[11px] text-slate-500 mt-1">{overallPct.toFixed(1)}% of microplan</p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50">
              ...
              <th className={th} onClick={() => handleSort('microplan_target')}>Target <SortIcon k="microplan_target" /></th>
              <th className={th} onClick={() => handleSort('achieved')}>Achieved <SortIcon k="achieved" /></th>
              <th className={th} onClick={() => handleSort('pct_complete')}>Progress <SortIcon k="pct_complete" /></th>
              <th className={th} onClick={() => handleSort('gap')}>Gap <SortIcon k="gap" /></th>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={r.facility_name} className={`border-b border-slate-100 hover:bg-[#f0f7fd] transition-colors ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'}`}>
                ...
                <td className="px-4 py-3"><ProgressBar pct={r.pct_complete} /></td>
                <td className="px-4 py-3">
                  <span className={`font-data text-[13px] font-semibold ${r.gap > 0 ? 'text-red-600' : 'text-green-600'}`}>
