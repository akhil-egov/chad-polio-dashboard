  ...
  return (
    <div className="space-y-4">
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-baseline">
                    <span className="font-condensed text-[10px] uppercase tracking-wide text-slate-500">{t('Households')}</span>
                    <span className="font-data text-[13px] font-semibold text-slate-700">{r.household_count.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between items-baseline">
                    <span className="font-condensed text-[10px] uppercase tracking-wide text-slate-500">{t('Eligible')}</span>
                    <span className="font-data text-[13px] font-semibold text-slate-700">{r.eligible_children.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between items-baseline">
                    <span className="font-condensed text-[10px] uppercase tracking-wide text-slate-500">{t('Vaccinated')}</span>
                ...
                <div>
                  ...
                  <p className={`font-data text-[12px] font-bold ${pctColor}`}>{r.pct_complete.toFixed(1)}% {t('coverage')}</p>
                </div>
              </div>
            </div>
          )
      ...
      <div className="bg-white border border-slate-200 rounded-md shadow-sm p-5">
        <p className="font-condensed text-[10px] font-bold tracking-[0.2em] uppercase text-slate-500 mb-3">
